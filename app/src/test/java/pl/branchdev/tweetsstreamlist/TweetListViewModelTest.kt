...
class TweetListViewModelTest {
    val mockedRepository: TwitterRepository = mock()
    val testConnectionSubject = PublishSubject.create<Boolean>()
    val mockedSchedulerProvider: SchedulerProvider = mock()
    val mockedTimeSpanBatchHandler: TimeSpanBatchHandler = mock()
    val mockedLogger: LoggerProvider = mock()
    val tweetTestSubject = PublishSubject.create<TweetDto>()
